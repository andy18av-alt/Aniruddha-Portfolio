...
export default function ContactForm() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({
        state: 'success',
        msg: 'Thank you. Your request has reached Aniruddha.'
      });
      ...
      setStatus({
        state: 'error',
        msg: 'Network error. Please email aniruddha.vanshiv@gmail.com directly.'
      });
    }
  };

  const fieldBase =
    "w-full bg-transparent border-b border-white/10 focus:border-white/40 outline-none text-[15px] text-white placeholder:text-neutral-600 py-3 px-0 transition-colors";
    ...
    <form onSubmit={onSubmit} className="relative" noValidate>
      ...
      <label className="block mt-6">
        ...
        <input
          ...
          value={form.company}
          onChange={onChange}
          placeholder="Company or role you're hiring for"
          className={fieldBase}
        />
      </label>
      ...
      <label className="block mt-6">
        ...
        <span className="block mt-1 text-[10px] text-neutral-600 tabular-nums">
      ...
      <div className="mt-8 flex items-center justify-between flex-wrap gap-4">
        ...
        <span className="text-[11px] text-neutral-600">
          Replies within 24–48 hours · IST
        </span>
      ...
      <AnimatePresence>
        ...
        {status.state === 'error' && (
          ...
          >
            ...
            <div className="text-[13px] text-rose-200/90">
              {status.msg}
            </div>
          </motion.div>
        )}
